...
// Keep the last `width` cells of a string (the opposite end of `truncateWidth`).
function tailWidth(str, width) {
  if (width <= 0) return ''
  let out = ''
  let w = 0
  const chars = [...str]
  for (let i = chars.length - 1; i >= 0; i--) {
    const cw = runeWidth(chars[i])
    if (w + cw > width) break
    out = chars[i] + out
    w += cw
  }
...
// Fit a filesystem path into `width` cells for display. A path is read from its
// tail — the project folder identifies the workspace, `/Users` does not — so the
// head is elided: whole trailing segments while they fit, otherwise the path's
// own tail fills the room so no space is wasted.
export function shortenPath(path, width) {
  const text = String(path ?? '')
  if (text.length === 0 || width <= 1) return ''
  ...
  for (let take = 1; take <= parts.length; take++) {
    const candidate = '…' + (hasSep ? sep : '') + parts.slice(parts.length - take).join(sep)
    if (displayWidth(candidate) > width) break
    best = candidate
  }
  // Segment-aligned elision reads better, but only when it uses the width it was
  // given; a bare `…\lib` in a wide row is worse than a clipped tail.
  if (best && displayWidth(best) * 2 >= width) return best
  const tail = tailWidth(text, width - 1)
  if (tail) return '…' + tail
  return best || truncateWidth(text, width)
}
...
// A stored image reference is usable in message content only while it still
// carries the media type every adapter reads to describe the image. The
// attachment store returns the complete reference; a partial object reduced to
// its id cannot be serialized into a request.
export function usableImageRef(ref) {
  return !!ref && typeof ref.attachmentId === 'string' && typeof ref.mediaType === 'string'
}

// Reconstruct ordered user content for one submitted composer value. `[Image N]`
// markers resolve to the saved attachment at that position, and the complete
// stored reference is what the image block carries: message content is the
// adapter's only description of an image, so a reference reduced to its id
// fails there rather than in the composer. A marker without a usable
// attachment is preserved as literal text so no user input silently
// disappears. An empty result falls back to the raw text so a submission is
// never empty.
export function userContentBlocks(text, resolved) {
  const blocks = []
  const parts = String(text).split(/\[Image (\d+)\]/g)
  for (let i = 0; i < parts.length; i++) {
    if (i % 2 === 0) {
      if (parts[i].length > 0) blocks.push({ type: 'text', text: parts[i] })
    } else {
      const ref = resolved[Number(parts[i]) - 1]
      if (usableImageRef(ref)) blocks.push({ type: 'image', attachment: ref })
...
export function toolSummary(name, args) {
  ...
  switch (label) {
    ...
    case 'ask_user_question': {
      // One question reads as the question itself; a batch reads as a count —
      // the full text is in the modal, not on the tool card.
      const questions = Array.isArray(obj?.questions) ? obj.questions : []
      if (questions.length === 0) return 'ask user'
      if (questions.length > 1) return `ask ${questions.length} questions`
      const text = String(questions[0]?.question ?? questions[0]?.header ?? '').trim()
      return text ? `ask ${text}` : 'ask user'
    }
    default: {
