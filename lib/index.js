...
export function apply(ctx, config) {
  ...
  async function forkAndSwitch({ cut, restoreFiles, label }) {
    ...
    if (restoreFiles) {
      const result = await restoreGitPaths(cwd, mutationPathsAfter(events, cut))
    ...
    const childHandle = await ctx.agents.create({
      sessionId: SessionId(newSessionId()),
      seed,
      meta: { cwd, parentSession: parentId, seedLength: seed.length },
    ...
    refreshContextMeter()
    void loadEffortSlider()
    app.addSystem(label + ' from ' + parentId + ' · ' + fileNote, 'info')
    app.setStatus('idle')
    app.closeRewind()
    void refreshRecentSessions()
    paint()
  ...
  function openRewindOverlay() {
    ...
    app.openRewind({ items, restoreOptions: REWIND_RESTORE_OPTIONS })
    paint()
  }
  ...
  async function applyRewindChoice(item, mode) {
    ...
    if (mode === 'code') {
      const cut = rewindCut(events, item.seq)
      const cwd = currentAgent.session.header.cwd ?? process.cwd()
      const result = await restoreGitPaths(cwd, mutationPathsAfter(events, cut))
      app.closeRewind()
      app.addSystem('files: ' + formatRestore(result), 'info')
      paint()
      return
    }
    await forkAndSwitch({
