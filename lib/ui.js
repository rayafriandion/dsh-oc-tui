...
import { displayWidth, truncateWidth, timeString, toolSummary, roughTokens, formatTokens, wrapText, shortenPath } from './util.js'
...
export const THEME = {
  ...
  workspaceBar: '16223a', // workspace strip under the header bar (between panel and element)
  ...
  inbox: '9fd0e8',          // pending-inbox box text (pale cyan)
  inboxBg: '1d4152',        // pending-inbox box background (cyan)
  steering: '9fd0e8',       // inline badge on a user turn that arrived as steering
  systemPrompt: 'b9c4dd',   // system-prompt box text (cool gray)
  systemPromptBg: '28324d', // system-prompt box background
...
// render() uses (composerX = sidebarW + 2 or 1; the box spans to cols - 1, with
// two columns of padding on each side). Shared so the approval prompt's line
// count and its painting can never disagree.
function composerTextWidth(cols, sidebarW) {
...
export class App {
  ...
  addUser(text, { steering = false } = {}) {
    this.detachInboxBlock()
    this.blocks.push(makeBlock('user', { text, steering }))
    this.assistantHeaderPending = true
    this.scroll = 0
  }
  ...
  _layout() {
    ...
    // to report keeps the row for the transcript.
    const statsH = this._statsStripVisible() ? 1 : 0
    ...
    if (this.pendingApproval) {
      const approval = this._approvalLines(composerTextWidth(cols, sidebarW), maxComposerRows)
    ...
    return { cols, rows, headerH, inputH, inputRowsVisible, suggestions, imageHintH, statsH, statusH, sliderH, transcriptTop, transcriptBottom, transcriptH, sidebarW, composerWidth, visual }
  }
    ...
    switch (block.kind) {
      case 'user': {
        out.push({ segs: [{ text: '  You  ', style: makeStyle({ fg: t.primary, bold: true }) },
          ...(block.steering
            ? [{ text: '⟲ steering  ', style: makeStyle({ fg: t.steering, bold: true }) }]
            : []),
      ...
      case 'sysnote': {
        // The session's system prompt (V3 `system/message`). Same collapsible
        // box treatment as the other notes, with its own neutral palette so it
        // never reads as a reminder or a compaction checkpoint.
        const palette = { fg: t.systemPrompt, bg: t.systemPromptBg }
        const collapsed = block.collapsed === true
        const hint = collapsed ? ' · click to expand' : ' · click to collapse'
        const headerSegs = [
          { text: '  ', style: makeStyle({ fg: palette.fg, bg: palette.bg }) },
          { text: collapsed ? '▸' : '▾', style: makeStyle({ fg: palette.fg, bg: palette.bg, bold: true }) },
          { text: ' system prompt' + hint, style: makeStyle({ fg: palette.fg, bg: palette.bg, bold: true }) },
        ]
        out.push({ segs: boxPad(headerSegs, width, palette.bg), note: { block } })
        if (!collapsed) {
          for (const line of renderMarkdown(block.text, t, width - 4)) {
      ...
      case 'inbox': {
        ...
        // entries wait for their own turn.
        const palette = { fg: t.inbox, bg: t.inboxBg }
        ...
        const headerSegs = clipSegs([
          { text: '  ', style: makeStyle({ fg: palette.fg, bg: palette.bg }) },
          { text: expanded ? '▾' : '▸', style: makeStyle({ fg: palette.fg, bg: palette.bg, bold: true }) },
          { text: ' pending input · ' + counts.join(' · ') + hint, style: makeStyle({ fg: palette.fg, bg: palette.bg, bold: true }) },
        ], width)
        out.push({ segs: boxPad(headerSegs, width, palette.bg), note: { block } })
        if (expanded) {
          const rows = [
          ...
          for (const { marker, kind, message } of rows) {
            const segs = boxPad([
              { text: '  ', style: makeStyle({ fg: palette.fg, bg: palette.bg }) },
              { text: marker + ' ', style: makeStyle({ fg: palette.fg, bg: palette.bg, bold: true }) },
              { text: kind + '  ', style: makeStyle({ fg: palette.fg, bg: palette.bg, dim: true }) },
              { text: inboxPreview(message.text, width - 20), style: makeStyle({ fg: t.text, bg: palette.bg }) },
            ], width, palette.bg)
            out.push({ segs, note: { block } })
          }
        }
  ...
  render() {
    const { cols, rows, headerH, inputRowsVisible, suggestions, imageHintH, statsH, transcriptTop, transcriptBottom, transcriptH, sidebarW, visual } = this._layout()
    this.hitRegions = []
    ...
    if (headerH > 1) {
      // Once a session is open the header carries the workspace it is rooted in
      // (the title screen shows it in its own centre block instead). The path is
      // shortened from the head so the project folder stays readable at any width.
      const branch = this.gitBranch ? 'git: ' + this.gitBranch : ''
      const branchRoom = branch ? displayWidth(branch) + 3 : 0
      const pathRoom = Math.max(0, cols - WORKSPACE_LABEL.length - 5 - branchRoom)
      const path = !this.titleScreen && this.workingDirectory && pathRoom >= WORKSPACE_MIN_PATH
        ? shortenPath(this.workingDirectory, pathRoom)
        : ''
      // The workspace strip keeps its own tone, so the row reads as a labelled
      // band under the brand bar instead of blending into the transcript. With
      // nothing to show (title screen, or no room for the path) the row stays on
      // the page background rather than tinting an empty line.
      const barBg = path ? t.workspaceBar : t.background
      screen.fill(0, 1, cols, ' ', makeStyle({ bg: barBg }))
      if (path) {
        let wx = screen.text(2, 1, WORKSPACE_LABEL, makeStyle({ fg: t.textMuted, bold: true, bg: barBg }))
        wx = screen.text(wx + 1, 1, path, makeStyle({ fg: t.text, bg: barBg }))
        if (branch && wx + 3 + displayWidth(branch) <= cols - 2) {
          screen.text(wx + 3, 1, branch, makeStyle({ fg: t.success, bg: barBg }))
        }
      }
    }
    ...
    if (this.titleScreen && this.blocks.length === 0) {
      ...
      const cwd = shortenPath(this.workingDirectory, Math.max(12, transWidth - 8))
    ...
    const approvalLines = this.pendingApproval
      ? this._approvalLines(composerTextWidth(cols, sidebarW), inputRowsVisible)
      : null
    ...
    // Left cluster: the model, plus the draft-token estimate while no usage has
    // been billed yet. Session tokens, speed, and cache-hit figures live on the
    // stats strip above the composer (one home per fact).
    ...
    // The context meter owns the right end of the row, and the left cluster is
    // sized to whatever space remains — the model name survives before the
    // draft count does when the row is crowded.
    ...
    if (this.overlay === 'rewind') this._paintRewind(screen, cols, rows)
    // The question modal owns the screen while a request is pending.
    this._questionCaret = null
    if (this.pendingQuestions) this._paintQuestions(screen, cols, rows)
    else if (this.statsOpen && !this.overlay) this._paintStats(screen, cols, rows)
  ...
  _paintStats(screen, cols, rows) {
    ...
    // Top border, title, hint, spacer, the body rows, bottom border.
    ...
    for (const row of body) {
      ...
      if (row.legend) {
        // The legend label is longer than the stats label column, so its figure
        // is right-aligned against the box instead of sharing that column.
  ...
  _paintQuestions(screen, cols, rows) {
    const t = THEME
    ...
    const w = Math.max(34, Math.min(78, cols - 8)) + 6
