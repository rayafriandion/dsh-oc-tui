...
// Label of the header row that names the workspace a session is rooted in.
...
export class App {
  ...
  render() {
    ...
    if (headerH > 1) {
      const workspaceStyle = makeStyle({ bg: t.background })
      screen.fill(0, 1, cols, ' ', workspaceStyle)
      ...
      if (!this.titleScreen && this.workingDirectory) {
        ...
        const path = shortenPath(this.workingDirectory, Math.max(0, cols - WORKSPACE_LABEL.length - 5 - branchRoom))
        if (path) {
          let wx = screen.text(2, 1, WORKSPACE_LABEL, makeStyle({ fg: t.textMuted, bold: true, bg: t.background }))
          wx = screen.text(wx + 1, 1, path, makeStyle({ fg: t.text, bg: t.background }))
          if (branch && wx + 3 + displayWidth(branch) <= cols - 2) {
            screen.text(wx + 3, 1, branch, makeStyle({ fg: t.success, bg: t.background }))
          }
        }
      }
    }
